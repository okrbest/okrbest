...
func TestMaskConditionValues_SharedOnlyText(t *testing.T) {
	...
	// Register a plain group — no access-control overhead for this group.
	// okrbest의 RegisterPropertyGroup은 이름만 받는다 (PropertyGroup에 Version 필드 없음).
	group, appErr := th.App.RegisterPropertyGroup(rctx, "masking_text_test_"+model.NewId())
...
func TestGetMaskedVisualAST_Wiring(t *testing.T) {
	...
	_, appErr := th.App.CreatePropertyField(rctx, field)
	require.Nil(t, appErr)
...
func TestMaskSimulationPolicyLiteralsForCaller_SourceOnly(t *testing.T) {
	...
	cpaGroup, gErr := th.App.GetPropertyGroup(rctx, model.CustomProfileAttributesPropertyGroupName)
	require.Nil(t, gErr)
...
func TestMaskSimulationPolicyLiteralsForCaller_ActualValueIndependentFromExpected(t *testing.T) {
	...
	group, gAppErr := th.App.RegisterPropertyGroup(rctx, "sim_mask_actual_test_"+model.NewId())
	...
	createdField, vAppErr := th.App.CreatePropertyField(rctx, field)
	require.Nil(t, vAppErr)
	...
	// The simulator pass uses GetPropertyGroup(CustomProfileAttributesPropertyGroupName)
