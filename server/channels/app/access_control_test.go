...
func TestRedactSimulationAttributesForCallerAccessModes(t *testing.T) {
	...
	createdPublic, vAppErr := th.App.CreatePropertyField(rctx, publicField)
	require.Nil(t, vAppErr)
...
func TestRedactSimulationAttributesForCallerFailClosed(t *testing.T) {
	...
	mockGroupStore.
		On("Get", model.CustomProfileAttributesPropertyGroupName).
...
func TestMergeStoredPolicyExpressions_ActionsEditableWhenNoMasking(t *testing.T) {
	...
	// okrbest: upstream은 앞선 CreatePropertyField(property v2)에서 appErr를 선언하지만
	// 우리는 v1 스토어 호출(fieldErr)로 적응해 두어 여기서 새로 선언한다.
