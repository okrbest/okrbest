...
func TestCreatePost(t *testing.T) {
	...
	// okrbest: upstream의 "creates post with type card" 서브테스트는 제거했다.
	// model.PostTypeCard가 우리 트리에 없기 때문이다 — 이 상수는 제외한
	// 48f2fd08(Integrated Boards MVP, #35796)이 도입했고 이 커밋 소관이 아니다.
...
func TestCollapsedThreadFetch(t *testing.T) {
	...
	t.Run("should only return root posts, enriched", func(t *testing.T) {
		...
		require.Nil(t, appErr)
		l, appErr := th.App.GetPostsForChannelAroundLastUnread(th.Context, channel.Id, user1.Id, 10, 10, true, true, false, nil)
		require.Nil(t, appErr)
		...
		// try extended fetch
		l, appErr = th.App.GetPostsForChannelAroundLastUnread(th.Context, channel.Id, user1.Id, 10, 10, true, true, true, nil)
		require.Nil(t, appErr)
		require.Len(t, l.Order, 1)
