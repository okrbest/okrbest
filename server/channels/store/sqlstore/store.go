...
// The DB migrations for the roles SchemeId columns were cherrypicked onto the v10.11 release branch but their numbers
// were changed to 142, 143 and 144. When upgrading from v10.11.17 to a newer release, the migrations that actually
// carry those numbers could never run due to migration version conflict in db_migrations table.
// This pre-migration functon fixes that issue by restoring the numbers this codebase assigns to those migrations,
// which lets the missing migrations run successfully and complete the upgrade.
// okrbest numbers them 161/162/163 while upstream uses 156/157/158, because 144-160 are taken by okrbest's own
// migrations (see server/channels/db/migrations/postgres).
func (ss *SqlStore) doRenumberRolesSchemeIdMigrations() error {
	query := `
UPDATE db_migrations
SET Version = CASE
    WHEN Version = 142 AND Name = 'add_schemeid_to_roles'    THEN 161
    WHEN Version = 143 AND Name = 'backfill_roles_schemeid'  THEN 162
    WHEN Version = 144 AND Name = 'add_roles_schemeid_index' THEN 163
END
