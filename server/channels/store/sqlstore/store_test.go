...
func TestDoRenumberRolesSchemeIdMigrations(t *testing.T) {
	...
	// Simulate an old install: replace the rows that Morph wrote at 161/162/163
	// with rows at the historical numbering 142/143/144.
	_, err = ss.GetMaster().Exec("DELETE FROM db_migrations WHERE Version IN (161, 162, 163, 142, 143, 144)")
	require.NoError(t, err)
	...
	require.NoError(t, ss.GetMaster().Select(&rows,
		"SELECT Version, Name FROM db_migrations WHERE Version IN (142, 143, 144, 161, 162, 163) ORDER BY Version"))

	assert.Equal(t, []row{
		{161, "add_schemeid_to_roles"},
		{162, "backfill_roles_schemeid"},
		{163, "add_roles_schemeid_index"},
	}, rows)
