...
document.addEventListener('DOMContentLoaded', function () {
    var hovers = document.querySelectorAll("div[data-okrbest-hover]");
    for (var i = 0; i < hovers.length; i++) {
        var element = hovers[i];
        element.addEventListener("mouseover", function(e) {
            onHover(e.currentTarget, e.currentTarget.getAttribute("data-okrbest-hover"));
        });
        element.addEventListener("mouseout", function(e) {
            onUnHover(e.currentTarget, e.currentTarget.getAttribute("data-okrbest-hover"))
        });
    }
    var clicks = document.querySelectorAll("div[data-okrbest-click], button[data-okrbest-click]");
    for (var i = 0; i < clicks.length; i++) {
        var element = clicks[i];
        element.addEventListener("click", function(e) {
            window.open(e.currentTarget.getAttribute('data-okrbest-click'), '_blank');
        });
    };
});