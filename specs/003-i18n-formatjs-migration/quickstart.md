...
## 시나리오 5 — 전체 코드베이스 `enforce-id` 준수 확인 (SC-005)

> 실행 결과 실제로는 배치가 필요 없었다(batches.md 참고) — 아래는 최종 상태를 확인하는 절차로 갱신됐다. `--throws`는 임의의 첫 경고(예: 중복 id)에서 멈추는 특성 때문에(research.md #7, #11) 이 검증에 부적합하다 — 대신 일반 lint 스캔 결과를 집계한다.

1. `.eslintrc.json`에 `formatjs/enforce-id`에 대한 `overrides` 예외가 없는지 확인한다.
2. `eslint --ext .js,.jsx,.tsx,.ts ./src --format json`로 전체를 스캔하고 결과에서 `"formatjs/enforce-id"` 발생 건수를 센다.
3. **기대 결과**: 0건. 이는 `en.json`의 모든 메시지가 명시적 id를 가진 상태임을 의미한다(단, id 값이 정적으로 추출 가능한지는 별도 확인 필요 — batches.md의 "동적 id" 사례 참고).
