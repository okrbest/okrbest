...
describe('components/admin_console/permission_policies/policy_details/PermissionPolicyDetails', () => {
    ...
    // okrbest: upstream also seeds TrustProxyDeviceIdentityHeader and
    // EnforceDeviceIDConsistency here. Our AccessControlSettings has neither —
    // the device-identity lineage is not in this fork, same gap as 020de7a6.
    // See docs/upstream-adapted-divergences.md.
    ...
    // for every expression the simple editor can represent. The named case
    // below is the original regression: a parenthesized multiselect
    // "has any of" OR-group was misclassified as complex by a stale local
    // helper and forced the editor into Advanced mode.
    //
    // okrbest: upstream's second named case covers a ranked-operator rule
    // (user.attributes.level >= "Senior"). We drop it — and the three ranked
    // cases in the table below — because the ranked-attribute lineage is not
    // in this fork. Our editors/shared.tsx lacks the ranked comparison
    // operators (IS_EXACTLY / IS_AT_LEAST / IS_GREATER_THAN / IS_AT_MOST /
    // IS_LESS_THAN) and isSimpleCondition only accepts == and !=, so a ranked
    // expression legitimately opens in Advanced mode here. See
    // docs/upstream-adapted-divergences.md.
