...
describe('KeepRemoveFlaggedMessageConfirmationModal', () => {
    ...
    // okrbest: the following guard requirements this fork committed to in
    // specs/011-data-spillage-report-ui. Upstream does not cover them.
    describe('okrbest guarantees', () => {
        test('keeps the typed comment when returning to the form step (FR-011)', async () => {
            Client4.generateFlaggedPostReport = jest.fn().mockReturnValue(new Promise(() => {}));
            ...
            await userEvent.type(screen.getByPlaceholderText('Add your comment here'), 'keep me');
            await userEvent.click(screen.getByRole('button', {name: 'Continue'}));

            await waitFor(() => {
                expect(screen.getByTestId('generating-back-button')).toBeVisible();
            });
            await userEvent.click(screen.getByTestId('generating-back-button'));

            await waitFor(() => {
                expect(screen.getByPlaceholderText('Add your comment here')).toHaveValue('keep me');
            });
        });

        test('aborts the in-flight report request when leaving the generating step (FR-012, FR-013)', async () => {
            let capturedSignal: AbortSignal | undefined;
            Client4.generateFlaggedPostReport = jest.fn().
                mockImplementation((_postId, _comment, _action, signal) => {
        ...
        test('does not put default focus on a destructive button (FR-017)', async () => {
            ...
            // Pressing Enter must never confirm an irreversible removal, so no
            // button may be autofocused and none may act as a submit button.
