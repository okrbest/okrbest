...
describe('getReportAProblemLink', () => {
    ...
    it('should return a mailto link to the support address if licensed with a paid SKU', () => {
        const state = {
            entities: {
                users: {
                    currentUserId: 'user1',
        ...
        const link = getReportAProblemLink(state);
        expect(link).toContain('mailto:sample@mail.com');
        expect(link).toContain(encodeURIComponent('Problem with OKR.BEST app'));
    ...
    it('should return the default unlicensed URL if unlicensed', () => {
        ...
        expect(getReportAProblemLink(state)).toContain('https://okr.best/pl/report_a_problem_unlicensed');
    });
...
describe('getDefaultReportAProblemMailtoLink', () => {
    ...
    it('should mention browser console logs when not on desktop app', () => {
        const link = getDefaultReportAProblemMailtoLink(baseState);
        expect(link).toContain(encodeURIComponent('browser console logs'));
    });

    it('should mention desktop app logs when on desktop app', () => {
        ...
        expect(link).toContain(encodeURIComponent('desktop app logs'));
